...
/**
 * Created by RPatel on 1/31/2016.
 */
public class DynamoDBMapperQueryScanExample {
    public static void main(String[] args) throws Exception {
        try {
            BasicAWSCredentials credentials = new BasicAWSCredentials(ApplicationProperties.getInstance().getProperty("aws.access_key_id"),
                    ApplicationProperties.getInstance().getProperty("aws.secret_access_key"));
            AmazonDynamoDBClient client = new AmazonDynamoDBClient(credentials);

            DynamoDBMapper mapper = new DynamoDBMapper(client);

            mapper.save(new LoanListing());
//
//            // Get a book - Id=101
//            GetBook(mapper, 101);
